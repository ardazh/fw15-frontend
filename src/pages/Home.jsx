...
const Home = () => {
  ...
  async function getDataEventCategory(name) {
    const { data } = await http().get("/events?limit=3", {
              ...
              >
                {({ handleBlur, handleChange, handleSubmit }) => (
                  <form
                    ...
                    className="flex justify-center items-center gap-4 bg-white w-[600px] h-[75px] rounded-2xl overflow-hidden"
                  >
                    <i>
                      {" "}
                      <FaSearch />{" "}
                    ...
                    <button
                      type="submit"
                      className="flex justify-center rounded-lg items-center bg-orange-400 h-11 w-11"
                    >
              ...
              {categories.map((category) => {
                return (
                  <div key={`categories-${category.id}`}>
                    <button
                      onClick={() => getDataEventCategory(category.name)}
                    ...
                    <div className="flex justify-center items-center gap-16 mt-12 mb-24">
                      ...
                      {partner?.picture && (
                        <img
                          className="w-full h-full object-cover"
                          src={
                            partner.picture.startsWith("https")
                              ? partner.picture
